...
awk 'BEGIN{bw=5;nb=int(60/bw)+1;bin[nb];count[nb];min[nb];max[nb];p=0.16}{if(NR==FNR){bin[$7]++}else{count[$7]++;if(count[$7]==int(bin[$7]*p))min[$7]=$8;if(count[$7]==int(bin[$7]*(1-p)))max[$7]=$8}}END{for(i=1;i<nb;i++)print (i-0.5)*bw,min[i],max[i]}' xx xx > UncorCorS_cont68.dat
rm x xx
