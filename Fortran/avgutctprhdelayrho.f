!! Genera archivo utctprh en bines de 1 hora y asignando la T y $\rho$ de 2 horas antes.
!!

		integer n,i,counter,status,lsup,binWidth,UTC
		integer date(9),hourCounter(24)
		real drho(24),drhomax(24), drhomin(24),rhoT2hr(3,2)
		open(unit=1,file='utctprhcavg.dat')
		open(unit=2,file='utctprhcavg-binsdlayrho.dat')
		open(unit=3,file='utctprhcavg-binsdlayrho-winter.dat')
		open(unit=4,file='utctprhcavg-binsdlayrho-summer.dat')
...
c     save values of rho and T for later use (in bin 2 hours later)
				rho=rhoT2hr(1,1)
				rhoT2hr(1,1)=rhoT2hr(2,1)
				rhoT2hr(2,1)=avgrho
				temp=rhoT2hr(1,2)
				rhoT2hr(1,2)=rhoT2hr(2,2)
				rhoT2hr(2,2)=avgtemp
			if(i.gt.2) then
			write(2,*)UTC-binWidth/2,temp,avgpres,rho,avgrhod,avghex6T5
c			write(2,*) UTC,avgtemp,avgpres,avgrho,avgrhod,avghex6T5,avgrm
		endif
		end do
	
c		call remsumorwin(0)
c		call remsumorwin(1)
