...
def the_all_mighty_function(file_auger, file_type, whereisit):
	...
	sec_= lambda theta: (np.sin(theta*np.pi/180.0))*(np.sin(theta*np.pi/180.0))
	sec_60=	sec_(60)
	with open(file_auger) as f:
		for line in f:
			utc,the,phi,S1000,dS1000,energy = line.split()
			sector=int(sec_(float(the))/(sec_60/5.0))
...
wherewillitbe="/home/ponci/Desktop/TesisIB/Coronel/Merged_Herald_Weather/Sec_above_1EeV/"
