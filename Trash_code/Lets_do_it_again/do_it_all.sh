...
#------------------------------------------------------------------------------------------------------
					...
					#bash ./Sh/utctprh_counter_script.sh "$project" "$old_weather" && echo "done with $old_weather " && echo "Paso 1, done."
#------------------------------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------------------------------
					#bash ./Sh/filter_herald.sh "$project" && echo "Paso 2, done."
#------------------------------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------------------------------
					#bash ./Sh/filter_herald_by_bad_period.sh  "$project" "$old_weather" && echo "Paso 3, done."
#------------------------------------------------------------------------------------------------------
...
#Hasta acá todavia no divido los hexagonos por 5 ni multipliqué por el area
#Tampoco hice la distincion de the menor a 60 o energia mayor a 1EeV
##--------------------------------------------------------------------------------------------------------------------------------
...
#Aunque necesito es rate global para luego tener el fit y ver si estoy haciendo bien las cosas. Así que este paso se va a dividir en dos: en el total y en dividir por secante
 			#Para crear el archivo por hora
 					bash ./Sh/Events_Counter_Files/merges_by_day.sh   "$project"  "$old_weather"  && echo "Paso 3.1 Hacer el fichero de eventos por hora"


 			#Para separar en archivos por bines de sin^2
