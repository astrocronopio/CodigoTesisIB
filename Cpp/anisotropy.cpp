...
void exposure_weight(vector<long double> & vect, unsigned long utci, unsigned long utcf, float freq)
{	std::vector<long double>xnhexhr(24);
	std::vector<long double>rnhexhr(24);


	unsigned long iutc, iutc0 = 1072915200;
	float t,p,r,rav,xnhex, xnhex5;
	int iw,ib, ihr;
	string line;
	...
	if(myweather.is_open())
	{
		while (!myweather.eof() ){			
			...
			liness>>iutc>>t>>p>>r>>rav>>xnhex>>xnhex5>> iw>>ib>>x1>>x2>>x3;

			if (iutc < utci || iutc > utcf) continue;

			if (iw<4 && ib>0.5)
			{
				x1=(float(iutc-iutc0)/3600.+31.4971*24/360)*fas;
				ihr= int(x1)%24;
				xnhexhr[ihr] += (3*xnhex5-xnhex)*0.5;
...
void ray_multifreq(int nf){
	...
	ofstream myfile ("Tesis_data_file_Eraw.txt");
